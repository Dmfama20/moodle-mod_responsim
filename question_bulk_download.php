...
$PAGE->set_url('/mod/responsim/question_bulk_download.php', array('id' => $cm->id));
...
if (empty($SESSION->bulk_questions)) {
...
if ($dataformat) {
    ...
    $filename = clean_filename('questions');

    $downloadquestions = new ArrayObject($SESSION->bulk_questions);
    $iterator = $downloadquestions->getIterator();

    \core\dataformat::download_data($filename, $dataformat, $fields, $iterator, NULL) ;
}

echo $OUTPUT->header();
echo $OUTPUT->heading('download questions');
echo $OUTPUT->download_dataformat_selector('download_questions', 'question_bulk_download.php');
echo $OUTPUT->footer();
